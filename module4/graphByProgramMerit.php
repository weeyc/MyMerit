...
$sql = "SELECT program.programName,program.programMerit 
FROM merit, program, student
WHERE merit.studentID=student.studentID AND merit.programID = program.programID AND merit.approval_status = 'approved' AND student.studentID ='$stdID'
GROUP BY program.programName
ORDER BY program.programMerit DESC";
    ...
    <script type="text/javascript">
      ...
      function drawStuff() {
        var data = new google.visualization.arrayToDataTable([
          ['programName', 'programMerit'],
   
            <?php 
             while($result = mysqli_fetch_assoc($fire)){
                 echo"['".$result['programName']."',".$result['programMerit']."],";
             }
            ?>
        ...
        var options = {
          ...
          chart: {
            title: 'Program Merit You Gained From The Program',
            subtitle: '' },
          axes: {
            x: {
              0: { side: 'top', label: 'programName'} // Top x-axis.
            }
          },
          bar: { groupWidth: "50%" }
        };
